...
class vin_web(NNobj):
    ...
    def __init__(self, model="WikiProj-Sanity", N = 6072, D = 279, emb_dim = 300,
                 dropout=False, devtype="cpu",
                 grad_check=False, reg=0, seed = 0, batchsize = 32,
                 report_gap = 900000, data_select = 1):
...
class VinBlockWiki(object):
	...
	self.R_bias = init_weights_T(N)
	self.params.append(self.R_bias)
        self.R_bias_full = self.R_bias.dimshuffle('x', 0)
	self.R = T.nnet.relu(self.R + self.R_bias_full)
        # initial value
	...
	self.M_bias = init_weights_T(N)
	self.params.append(self.M_bias)
	self.M_bias_full = self.M_bias.dimshuffle('x',0)
	self.page_map = T.nnet.relu(self.page_map + self.M_bias_full)
