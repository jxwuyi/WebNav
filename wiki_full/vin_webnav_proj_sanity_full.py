...
class vin_web(NNobj):
    ...
    def __init__(self, model="WikiProj-SanityF", N = 6072, D = 279, emb_dim = 300,
                 dropout=False, devtype="cpu",
                 grad_check=False, reg=0, seed = 0, batchsize = 32,
                 report_gap = 1300000, data_select = 1):
...
class VinBlockWiki(object):
	...
	self.last_bias_full = T.extra_ops.repeat(self.last_bias, Q_in.shape[0], axis=0)
