...
class vin(NNobj):
    ...
    def run_training(self, stepsize=0.01, epochs=10, output='None',
        ...
        if (prm.only_predict):
            ...
            test_n = len(test_entry) / 4 # to make things faster
...
class VinBlockWiki(object):
    ...
    def __init__(self, Q_in, S_in, N, D, emb_dim,
        ...
        :type edges: np.array, int32
        :param edges: flatten adjacency matrix, of shape [N_pages * D]
        ...
        self.edge = np.repeat(self.edge, batchsize, axis = 0)
	...
	#self.add_R = T.extra_ops.repeat(self.R_full, A, axis = 2)        
        ...
        for i in range(k - 1):
            ...
            self.nq = self.V[T.extra_ops.repeat(T.arange(batchsize), N * D),
                             self.edge.flatten()] # (batchsize * N * D)
        ...
        if (prm.diagonal_action_mat): # simple diagonal transition matrix
            if (prm.final_scale):
                self.w_o = theano.shared((np.ones((1, D))*0.1).astype(theano.config.floatX))#init_weights_T(1, D)
